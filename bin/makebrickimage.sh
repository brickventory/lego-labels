...
PARTS="$HOME/Lego/LDraw/parts"

# what directory to save the files in, relative from the location from where this script is run
SAVE_DIR="../images/"
PART_NUMBER=$1
...
GENERAL_PARAMS="-Antialias=0 -LineSmoothing=1 -TransDefaultColor=0 -DefaultColor3=0xFFFFFF -BackgroundColor3=0xFFFFFF -ProcessLDConfig=1 -MemoryUsage=2 -ShowAxes=0"
...
SAVE_PARAMS="-AutoCrop=1 -ModelSize=200 -SaveActualSize=1 -SaveZoomToFit=1 -SaveWidth=256 -SaveHeight=256 -IgnorePBuffer=0 -SaveAlpha=1"

PARAMS="$GENERAL_PARAMS $GEOMETRY_PARAMS $EFFECTS_PARAMS $PRIMITIVES_PARAMS $SAVE_PARAMS"

# ------------------------------------------------------------
# THIS PART SETS THE PERSPECTIVE THE PART WILL BE RENDERED
# ------------------------------------------------------------

PERSPECTIVE_DEFAULT="-FOV=0.1 -DefaultZoom=0.95 -DefaultLatLong=30,45"

# these lines set the perspective to the desired angle - choose wisely
PERSPECTIVE0="-FOV=0.1 -DefaultZoom=0.95 -DefaultMatrix=0.866025,0,-0.5,-0.286788,0.819152,-0.496732,0.409576,0.573576,0.709406"
...
# -----------------
$LDVIEW_APP $PARTS/$PART_NUMBER.dat -SaveSnapshot=$SAVE_DIR$PART_NUMBER.png $PARAMS $PERSPECTIVE_DEFAULT